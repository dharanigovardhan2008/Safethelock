...
export function AddProjectModal({ isOpen, onClose, onSave, editingProject }: AddProjectModalProps) {
  ...
  const InputRow = ({ icon: Icon, label, field, placeholder }: any) => (
    <div className="flex-1 space-y-1.5">
      ...
      <input type="text" value={(formData as any)[field]} onChange={(e) => setFormData({...formData, [field]: e.target.value})} className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-3 py-2.5 text-sm text-slate-200 focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all" placeholder={placeholder} />
    </div>
  );
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="flex gap-4">
            <InputRow icon={FaGithub} label="GitHub Email" field="github" placeholder="user@github.com" />
            <InputRow icon={SiFirebase} label="Firebase Email" field="firebase" placeholder="user@firebase.com" />
          </div>
          
          <div className="flex gap-4">
            <InputRow icon={SiVercel} label="Vercel Email" field="vercel" placeholder="user@vercel.com" />
            <InputRow icon={FaAws} label="AWS Email" field="aws" placeholder="user@aws.com" />
          </div>
