...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  // Exact match to PasswordCard's working copy function
    ...
    <div className="flex items-center justify-between py-1.5 border-b border-white/5 last:border-0 pointer-events-auto">
      ...
      {value && (
        <button 
          ...
          className="p-1.5 text-slate-500 hover:text-white transition-colors relative z-50 active:scale-95"
        >
    ...
    <div className="relative group w-[340px] h-[240px] transition-transform duration-300 hover:-translate-y-2" style={{ perspective: '1200px' }}>
      <motion.div className="w-full h-full relative rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.6)] group-hover:shadow-[0_30px_60px_rgba(0,0,0,0.7)] transition-shadow duration-300 ring-1 ring-white/20" initial={false} animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ duration: 0.7, type: "spring", stiffness: 200, damping: 20 }} style={{ transformStyle: 'preserve-3d' }}>
        ...
        <div onClick={() => handleCardClick(true)} className={cn("absolute inset-0 rounded-2xl overflow-hidden cursor-pointer", gradient)} style={{ backfaceVisibility: 'hidden' }}>
          ...
          {/* Matches PasswordCard structure: pointer-events-none on wrapper, auto on button */}
            ...
            <div className="flex justify-between items-start">
              ...
              {!isEditMode && (
                <button 
                  ...
                  className="pointer-events-auto p-2 bg-black/40 hover:bg-rose-600 rounded-full text-white/80 hover:text-white transition-all opacity-0 group-hover:opacity-100 border border-white/20 z-50 active:scale-95"
        ...
        <div onClick={() => handleCardClick(false)} className="absolute inset-0 rounded-2xl flex flex-col overflow-hidden cursor-pointer bg-[#0a0f1a] ring-1 ring-white/10" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
          ...
          {/* Wrapper has pointer-events-none, inner controls have pointer-events-auto */}
            ...
            <div className="mt-auto flex justify-between items-center pt-2 pointer-events-auto">
              <div className="flex gap-2 relative z-50">
                <button 
                  ...
                  className="p-2 bg-white/5 hover:bg-indigo-500 rounded-lg text-slate-400 hover:text-white transition-colors active:scale-95"
                >
                  <Edit2 size={14} />
                </button>
                <button 
                  onPointerDownCapture={(e) => { e.stopPropagation(); onDelete(id); }}
                  className="p-2 bg-white/5 hover:bg-rose-500 rounded-lg text-slate-400 hover:text-white transition-colors active:scale-95"
                >
                  <Trash2 size={14} />
                </button>
              </div>
              {liveUrl && (
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer" 
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-white rounded-lg text-[10px] font-bold tracking-widest uppercase transition-colors relative z-50 active:scale-95"
                >
