...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const ServiceRow = ({ icon: Icon, name, value, field }: any) => (
    ...
    <div className="flex items-center justify-between py-1.5 border-b border-white/5 last:border-0">
      ...
      {value && (
        <button onPointerDownCapture={(e) => handleCopy(e, value, field)} className="p-1.5 text-slate-500 hover:text-white transition-colors relative z-50">
        ...
        <div onClick={() => handleCardClick(true)} className={cn("absolute inset-0 rounded-2xl overflow-hidden cursor-pointer", gradient)} style={{ backfaceVisibility: 'hidden' }}>
          ...
          <div className="absolute inset-0 p-5 flex flex-col z-10 pointer-events-none">
            <div className="flex justify-between items-start">
              ...
              {!isEditMode && <button onPointerDownCapture={(e) => { e.stopPropagation(); onDelete(id); }} className="pointer-events-auto p-2 bg-black/40 hover:bg-rose-600 rounded-full text-white/80 hover:text-white transition-all opacity-0 group-hover:opacity-100 border border-white/20 z-50"><Trash2 size={14} /></button>}
            </div>
          ...
          <div className="px-4 py-2 pb-4 flex-1 flex flex-col relative z-10">
            ...
            <div className="mt-auto flex justify-between items-center pt-2">
              <div className="flex gap-2 relative z-50">
                <button onPointerDownCapture={(e) => { e.stopPropagation(); onEdit(id); }} className="p-2 bg-white/5 hover:bg-indigo-500 rounded-lg text-slate-400 hover:text-white transition-colors"><Edit2 size={14} /></button>
                <button onPointerDownCapture={(e) => { e.stopPropagation(); onDelete(id); }} className="p-2 bg-white/5 hover:bg-rose-500 rounded-lg text-slate-400 hover:text-white transition-colors"><Trash2 size={14} /></button>
              </div>
              {liveUrl && (
                <a onPointerDownCapture={(e) => e.stopPropagation()} href={liveUrl.startsWith('http') ? liveUrl : `https://${liveUrl}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-white rounded-lg text-[10px] font-bold tracking-widest uppercase transition-colors relative z-50">
