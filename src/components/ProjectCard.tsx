...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const handleCopy = async (e: React.MouseEvent | React.PointerEvent, text: string, field: string) => {
    e.preventDefault();
    e.stopPropagation();
    try {
  ...
  const handleEdit = (e: React.MouseEvent | React.PointerEvent) => {
    e.preventDefault();
    e.stopPropagation();
    onEdit(id);
  };
    ...
    <div className="relative group w-[340px] h-[240px] transition-transform duration-300 hover:-translate-y-2" style={{ perspective: '1200px' }}>
      <motion.div className="w-full h-full relative rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.6)] group-hover:shadow-[0_30px_60px_rgba(0,0,0,0.7)] transition-shadow duration-300 ring-1 ring-white/20" initial={false} animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ duration: 0.7, type: "spring", stiffness: 200, damping: 20 }} style={{ transformStyle: 'preserve-3d' }}>
        ...
        <div onClick={() => handleCardClick(true)} className={cn("absolute inset-0 rounded-2xl overflow-hidden cursor-pointer", gradient)} style={{ backfaceVisibility: 'hidden' }}>
          ...
          {isEditMode && (
            <div className="absolute inset-0 z-[9999] bg-slate-950/80 backdrop-blur-md flex flex-col items-center justify-center border-[3px] border-indigo-400 rounded-2xl animate-in fade-in duration-200 shadow-[inset_0_0_50px_rgba(99,102,241,0.3)] pointer-events-none">
          ...
          <div className="absolute inset-0 p-5 flex flex-col z-10 pointer-events-none">
            <div className="flex justify-between items-start">
              <div className="p-1.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 shadow-inner drop-shadow-lg pointer-events-none">
            ...
            <div className="flex flex-col mt-auto mb-4 pointer-events-none">
            ...
            <div className="flex justify-between items-end border-t border-white/20 pt-3 pointer-events-none">
